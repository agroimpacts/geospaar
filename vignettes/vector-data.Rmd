...
<span style="color:blue">Exercise 13</span><a name="exercise13"></a>: ___Run the `erase` example again, but apply it to the `districts_int_spp` and `spp_alb` without coercing them to `SpatialPolygons` to see the error. How do you think we could fix those errors? No need to do it, but just want you to think how you might do it. Hints: look at the function `spChFids` and at Kristen's solution in this [post](https://groups.google.com/forum/#!topic/davis-rug/BmchhejJm4M)___ [Exercise index](#unit3exercises) 
