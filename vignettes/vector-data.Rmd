...
We can also simply convert `farmers` coordinates to `SpatialPoints`, a simpler spatial class that lacks the accompanying `data.frame`. We are not going to show that here, because that is your first exercise to complete 


<span style="color:blue">Exercise 1</span><a name="exercise1"></a>: 
___Figure out how to convert `farmers` to just `SpatialPoints`, rather than a `SpatialPointsDataFrame`___ ([Exercise index](#unit1exercises))

Let's move on and keep working with `farmers` as a `SpatialPointsDataFrame`, so your next exercise is to: 

<span style="color:blue">Exercise 2</span><a name="exercise2"></a>: ___Promote `farmers` back to a `SpatialPointsDataFrame`___ ([Exercise index](#unit1exercises))
...
Now, your next exercise is to: 

<span style="color:blue">Exercise 3</span><a name="exercise3"></a>: ___Use `readOGR` to load `districts.shp` into a new object called `districts`, and then figure out the classes of both `roads` and `districts`. Also look at the structure of each object___ ([Exercise index](#unit1exercises))
...
Right, that's the end of this section, but you are left with a final exercise to do: 

<span style="color:blue">Exercise 5</span><a name="exercise5"></a>: 

___1. Adapt the code above to reproject the spatial objects in `spdat` to the crs used by `roads`___

___2. Recreate the same plots using this newly reprojected dataset (i.e. first plot the districts then add the roads and farmers to it___ ([Exercise index](#unit1exercises))
...
As you probably closed this project between Section 1 and 2, you will also need to load back in the `roads`, `districts`, and `farmers` data, and, if necessary, promote them to `Spatial*` classes. The code in [Section 1](#section1) will show you how to do that. 

There are a total of 6 exercises in this section:
...
The resulting centroids in `dist_cent` are of class `r class(dist_cent)`. Note the use of "byid = TRUE" as an argument.  If we set that to `FALSE`, or didn't specify the argument at all, it would have returned a single set of coordinates for the centroid of Zambia. Now, here's the first exercise for this unit: 

<span style="color:blue">Exercise 6</span><a name="exercise6"></a>: ___Redo the code above so that you get just the centroid of Zambia (i.e. that single point I just mentioned), and then plot that onto the district map of Zambia (i.e. the map above, but with just one red point in Zambia' geographic center___ ([Exercise index](#unit2exercises))
...
Okay, here's another exercise: 

[<span style="color:blue">Exercise 7</span><a name="exercise7"></a>] ___Figure out which district is the closest to the centroid of Zambia, and how far that distance is in kilometers___ ([Exercise index](#unit2exercises))
...
The histograms plotted above have some graphical parameters added to them that might be new to you ("mgp", "cex.main", "cex.lab", "cex.axis"). 

[<span style="color:blue">Exercise 8</span><a name="exercise8"></a>] ___Rerun those plots, changing the values (at least one time), to the "mgp", "cex.main", "cex.lab", and "cex.axis" arguments.___ ([Exercise index](#unit2exercises))
...
That's index-based subsetting. We can also do it using values stored within the rows and columns of the data. But to do that, we are first going to enhance those data a bit.  
...
That brings us to our next exercise, for which I will first do some more work to help: 

```{r}
roads$roadnum <- 1:length(roads)  # new identifier, because of NAs in "roadid"
```

<span style="color:blue">Exercise 9</span><a name="exercise9"></a>: ___Adapt the code you see above to calculate road lengths (in km) in `roads`, and merge the resulting `data.frame` containing the road lengths and "roadnum" identifier (the merge key) onto `roads`, to create a new `roads2` object. Plot the histogram of those lengths, making the bars blue___ ([Exercise index](#unit2exercises))
...
As an exercise, create new district subsets by use a different grep pattern match on the district names (look at `districts_alb2$distName` for some ideas), and by changing the area used as a threshold (e.g. make it 1 million ha) size, and the replot the results. This leads us to our penultimate exercise for this section, which should be fairly easy (you will probably just have to read `?rgb` to figure it out)

<span style="color:blue">Exercise 10</span><a name="exercise10"></a>: ___Recreate the plot above, but instead of a partially transparent blue, use a partially transparent green for the plot of `dists_gt2m`___ ([Exercise index](#unit2exercises))
...
Okay, now for our final exercise of this section:

<span style="color:blue">Exercise 11</span><a name="exercise11"></a>: ___Taking the split-apply-combine code above as an example, find___

___1. The 15th, 25th and 75th, and 85th percentile area value for districts___

___2. Apply the split apply combine, selecting districts falling between the 15th and 25th percentiles of area, and between the 75th and 85th percentile areas, calculating the centroids of the selected districts___  

___3. Plot the centroids as above___ ([Exercise index](#unit2exercises))
...
In this section we will start to do some spatial operations (overlays/extractions, intersections, unions, and differences), and also learn how to create polygons from scratch. We are going start working with the `raster` package as well, which seems strange given that we are working with vectors, but the reason why will become clear below. 

There will be a total of X exercises:
...
spp_alb <- SpatialPolygonsDataFrame(spp_alb, data.frame("ID" = 1))
...
The first thing we will do with our made-up polygon is figure out which districts it intersects. What we are going to do in this case is the ArcGIS equivalent of select by spatial location (I hope I am remembering my ESRI terminology correctly--it's been a while since I looked at it) 
...
The `over` function returns the subset of `districts_alb2` that `spp_alb` overlies, returning the relevant subset of `districts_alb2`'s data.frame within a list. What `over` does, from `?sp::over`:
...
Another way of figuring out the same answer is with an `rgeos` function, `gIntersects`
...
So that gives somewhat different results. First, `gIntersects` returns a logical value, giving a `TRUE` or `FALSE` for each district in `districts_alb2` as to whether it is intersected by `spp_alb`. We can use the `which` operator to identify the index of the intersecting districts, and then use the indices to subset out the matching districts, as follows.

```{r}
spp_alb_int <- which(gIntersects(spgeom1 = spp_alb, spgeom2 = districts_alb2,
                                 byid = TRUE))
districts_int_spp <- districts_alb2[spp_alb_int, ]

par(mar = c(0, 0, 0, 0))
plot(districts_alb2, col = "grey")
plot(districts_int_spp, add = TRUE, 
     col = rainbow(n = length(districts_int_spp)))
plot(spp_alb, add = TRUE, border = "grey20", col = "grey20", density = 10,
     angle = 45, lwd = 1.5)
```

In the code above, you will see that we slightly revised the code, by putting the `gIntersects` inside the `which` function, and capturing the output in `spp_alb_int`, which we then used to subset `districts_alb2` into a new object:  `districts_int_spp`. 

In the plot commands function, you will also see a new way of selecting colors, using the `rainbow` function. In this case, we tell `rainbow` to select "n" colors (here equal to the number of districts in `districts_int_spp`), giving us a different color for each of the selected districts. We also overlay `spp_alb` onto the districts it intersects, using angled cross-hatching in place of a fill color (set using the "density" and "angle" arguments). Cross-hatching is apparently considered a no-no by visualization experts, however, because it can produce perception distortions, but now you know how to do it. 

[Back to top](#vectorintro)

### Intersections

We have just seen how `gIntersects` can be used to identify intersecting polygons. Now let's look at what `gIntersection` does. 

```{r}
spp_int_distrs <- gIntersection(spgeom1 = spp_alb, spgeom2 = districts_alb2, 
                                byid = TRUE)

par(mar = rep(0, 4))
plot(districts_alb2, col = "grey")
plot(spp_int_distrs, col = rainbow(n = length(spp_int_distrs)), add = TRUE)
```

Seems pretty clear what `gIntersection` does, and how it differs from `gIntersects`. However, note that you have to use the "byid = TRUE" option if you want to retain the detail of the intersected district boundaries, else you would get this:
...
Just a single filled polygon, not what we were hoping to see. 

Here's a much friendlier way of doing an intersection, using (somewhat counterintuitively) a function from the `raster` package.  The developers of the `raster` package have created a more user friendly set of certain `rgeos` functions, so we are going to demonstrate those here, in contrast to the `gIntersection`

```{r}
library(raster)
spp_int_distrs <- gIntersection(spgeom1 = spp_alb, spgeom2 = districts_alb2, 
                                byid = TRUE)
spp_int_distrs2 <- intersect(x = spp_alb, y = districts_alb2)
class(spp_int_distrs2)[1]
class(spp_int_distrs)[1]
```

It turns out that `raster` has a function (`intersect`) that does the same job as `rgeos` (go ahead and plot `spp_int_distrs2`, and you will see) but even better, as it retains the relevant information from any `data.frame`(s) in the objects being intersected; `gIntersection` only returns the intersected geometries, and you need to do a great deal more work to recover and then join the accompanying attributes to the geometries.  


> An aside: These last example demonstrates both a strength and weakness of R, in my opinion--R's strength is that you can usually find multiple solutions to solve a particular problem; R's weakness is that choosing the best solution can be tricky, given the profusion of choice, and you might end up trying something that takes up a lot of your time. 
...
> And the code above demonstrates more package differences. Detaching/unloading `raster`, as done above, and then printing out farmers (just the first two lines, to avoid a very long print of all coordinates) provides a much more longer and less convenient output (produced by `sp`) than the one produced by the `raster` package after it is re-loaded.  

### Aggregation/Dissolve

Now that we have intersected boundaries, let's dissolve them.  

```{r, fig.width=5, fig.height=5}
spintd_agg <- aggregate(x = spp_int_distrs2, by = "ID") 
spintd_agg2 <- aggregate(x = spp_int_distrs2)
spintd_agg3 <- aggregate(x = spp_int_distrs2, dissolve = FALSE)
spintd_agg4 <- aggregate(x = spp_int_distrs2, by = "ID", dissolve = FALSE)

l <- list(spintd_agg, spintd_agg2, spintd_agg3, spintd_agg4)
par(mfrow = c(2, 2), mar = c(0, 0, 0, 0))
for(i in l) {
  plot(districts_alb2, col = "grey")
  plot(i, col = rainbow(n = length(i)), add = TRUE)
}

```

We have three variants on `aggregate` shown above.  `spint_agg` resulted from "by = "ID"", which aggregates polygons according to the values the "ID" column. `spint_agg2` did not have that argument passed to it, `spint_agg3` had "dissolve = FALSE" as an argument, and `spint_agg4` is the same as the previous plus the "by = "ID"" argument. The plots shows that the first two look identical and that they were dissolved. The second two look like they weren't dissolved at all. So what are the differences? 

> Function note: There are two versions of `aggregate` that we could have used here. The `raster` package has its own version, which is simpler than the one provided by `sp`. Since we loaded `raster` last into our environment (remember how to confirm that? use `search()` to check the load order), the version we are using here is the one provided by `raster`, which works on rasters as well as `SpatialPolygons`. 

```{r}
sapply(l, class)
```

Okay, so 1 differs from 2 in that the former has a `data.frame`, and the same holds for 3 and 4. So how do 1/2 differ from 3/4? 

```{r}
t(sapply(l, function(x) {
  c("N poly groups" = length(x),  # 1
    "N polys in group" = length(slot(slot(x, "polygons")[[1]], "Polygons"))) # 2
}))
```

> Code explanation: The somewhat complex bit of code above uses `sapply` to loop through each permutation of the aggregations, and first line in the body (# 1) queries how many polygons groups each element of `l` has, while the second line (# 2) does this:  
>
> - `slot(x, "polygons")[[1]]` grabs the first group of polygons (there is only one in this example) living under the "polygons" slot of the `SpatialPolygonsDataFrame` `x` (anyone of the four aggregated objects in `l`). There is only one "polygons" slot in any `SpatialPolygonsDataFrame`. 
>
> - `slot(<snip>, "Polygons")` accesses the "Polygons" slot from the first group of polygons (which was extracted as described above, the code for which fits into the place where <snip> is now). There can be multiple polygons living in each polygon group, one or more of which may define holes in another polygon(s) in the group, or which many simply be polygons representing parts of a more complicated thing that is considered a ___single feature___, such as an island chain that has one name (e.g. the State of Hawaii, or Indonesia).  
>
> - That is why there is this awkward and complicated structure to `SpatialPolygons*`

What does the output of that `sapply` tell us?  That each of objects in `l` has just one polygon group, and that for the first two objects there only have one polygon per polygon group. The reason for that is that the aggregation ___dissolved___ the district boundaries. The second two were aggregated differently--they were not dissolved, but instead aggregated so that the polygons are now gathered into a single polygon group that is taken to represent the components of a single feature (as described above). 

Now, for some more advanced coding. The `sapply` above is somewhat half-baked, because it assumes that the `SpatialPolygons*` in `l` only have one polygon group each.  Here is how we would make that query more programmatic, i.e. if we didn't want to assume there was only one polygon group per object in `l`: 
...
do.call(rbind, lapply(1:length(l), function(x) {  
  ...
  ps <- t(sapply(1:length(p), function(y) {  # loop through p's poly groups
    a <- length(slot(p[[y]], "Polygons"))  # find out # polys in each poly group
    c("poly group" = y, "N polys in group" = a) # combine w/ y, poly grp counter
  }))  # transpose sapply results to get variables as columns
  cbind("object#" = x, ps)  # cbind x with ps, giving a label for object 
}))  
```

I am not going to explain this code too much, but what it does is give you a way of creating the same result as the previous `sapply`, plus some information about which object in the list corresponds to the results. If you run the two commented out lines above the code, you can use that to step through the inner parts of the function, which is more complex than you have seen before because it nests `sapply` (wrapped in `t`) within an `lapply`. That will help you understand the code better.  

However, the main point of showing that code is so that you can recycle it for an exercise:

<span style="color:blue">Exercise 12</span><a name="exercise12"></a>: ___Recreate the list `l` so that it contains just two objects: `spp_int_distrs2` and `spintd_agg4`. Rerun the code directly above with the new `l`. What do the results tell us about the structures of the two objects in `l`?___ [Exercise index](#unit3exercises) 

By the way, before we go into the next section, just wanted to mention that the `gUnaryUnion` dissolves the same way that `aggregate` does by default (e.g. as done to make `spintd_agg2` above), producing just `SpatialPolygons` output.  

```{r}
gUnaryUnion(spgeom = spp_int_distrs2)
spintd_agg2
```

### Disaggregation

We have shown how you can aggregate `SpatialPolygons*`. Now let's see about disaggregating polygons that have been aggregated. 

```{r}
spintd_dagg <- disaggregate(x = spintd_agg) 
spintd_dagg4 <- disaggregate(x = spintd_agg4)

l <- list(spintd_dagg, spintd_dagg4, spintd_agg4)
sppoly_structure(l)
?sppoly_structure
```

In the above, we have applied `disaggregate` to the dissolved aggregate `spint_agg`, and to the version of the aggregation in which the districts were aggregated into a single polygon group (`spintd_agg4`). We combine the results of the two disaggregation into a list, together with `spintd_agg4`, and then run `sppoly_structure` on it. 

> Wait, what is `sppoly_structure`? That complicated thing I asked you to run for [Exercise 12](#exercise12)--I added it as a function to the `geospaar` package so that I didn't have to keep copying a big chunk of code that I want to use--that right there (the need to repeatedly reuse some code) is the main reason for writing and documenting a package function. Have a look at `?sppoly_function`.

The results tell us that you can't really disaggregate `SpatialPolygons*` that have been dissolved, but you can split up a polygon group containing multiple polygons into multiple polygon groups, each with a single polygon in it. Compare the results for object#s 2 (`spintd_dagg4`) and (`spintd_dagg4`) 3 in the output from `sppoly_structure` above to see how disaggregation changes the structure of an aggregated polygon. 

[Back to top](#vectorintro)

### Differences

The two preceeding sections gave a rather long look at aggregation/disaggregation and how to query slots. The intention there was to better explore the fairly complicated structures of `SpatialPolygons*`, and how slots are queried. Please ask questions in class if this stuff doesn't make sense. 

Now we are going to move on to do look at another bread and butter operation, differencing (or erasing).

```{r, fig.width=7, fig.height=3}
d1a <- gDifference(spgeom1 = districts_int_spp, spgeom2 = spp_alb, byid = TRUE)
d1b <- gDifference(spgeom1 = districts_int_spp, spgeom2 = spp_alb)
d2 <- erase(x = as(districts_int_spp, "SpatialPolygons"),
            y = as(spp_alb, "SpatialPolygons"))

l <- list(d1a, d1b, d2)
titles <- c("gDifference by ID", "gDifference with dissolve", "raster::erase")
par(mfrow = c(1, 3), mar = c(0, 0, 1, 0))
for(i in 1:length(l)) {
  plot(l[[i]], col = rainbow(n = length(l[[i]])), main = titles[i])
}
```

That shows `rgeos::gDifference` applied two ways to cut out the area of `spp_alb` from the districts it ___intersects___, one in which the remainder shape of each district is preserved (with the "byid = TRUE" argument), the other in which the remainders of the districts are dissolved. The function `raster::erase` does the same as the first version of `gDifference`. In this case, we coerce the two input objects to `SpatialPolygons` to deal with an error `erase` was throwing about row.names in the `data.frame` not matching the polygons IDs.  

<span style="color:blue">Exercise 13</span><a name="exercise13"></a>: ___Run the `erase` example again, but apply it to the `districts_int_alb` and `spp_alb` without coercing them to `SpatialPolygons` to see the error. How do you think we could fix those errors? No need to do it, but just want you to think how you might do it. Hints: look at the function `spChFids` and at Kristen's solution in this [post](https://groups.google.com/forum/#!topic/davis-rug/BmchhejJm4M)___ [Exercise index](#unit3exercises) 


> Plotting tip: note in the code above how we add titles to the plots, and also tweak the margins to add space to the top margin so that those titles can be  displayed. 

[Back to top](#vectorintro)

### Unions

We have already seen a `gUnaryUnion`, which is basically a dissolve. Now let's look at unions in the way you are probably more used to seeing them in traditional GIS. 

```{r, fig.width=7, fig.height=3}
spp_dist_uni1 <- union(x = spp_alb, y = districts_int_spp)
spp_dist_uni2 <- gUnion(spgeom1 = spp_alb, spgeom2 = districts_int_spp, 
                        byid = TRUE)
spp_dist_uni3 <- gUnion(spgeom1 = spp_alb, spgeom2 = districts_int_spp)

l <- list(spp_dist_uni1, spp_dist_uni2, spp_dist_uni3)
titles <- c("raster::union", "gUnion byid", "gUnion dissolve")
par(mfrow = c(1, 3), mar = c(0, 0, 1, 0))
for(i in 1:length(l)) {
  dlabs <- sapply(slot(l[[i]], "polygons"), function(x) slot(x, "ID"))
  plot(l[[i]], col = rainbow(n = length(l[[i]])), main = titles[i])
  text(l[[i]], labels = dlabs)
}
...
Three different ways of doing a union, the first with raster::union, which as the `?union` for it describes, does this when applied to SpatialPolygons*:

> Overlapping polygons (between layers, not within layers) are intersected, other spatial objects are appended. Tabular attributes are joined.

You can see how `union` joins the attributes above.  

The other two ways are with `gUnion`, which allows unioning by ID (middle map), or a complete dissolve. As with the earlier `rgeos` functions, the operation leaves behind attributes. And you will see that the union-by-id result looks rather different than the one produced by `raster`, seeming to dissolve `spp_alb` into the entirety of one districts but only the overlapping parts of the other districts. Keep these alternative behaviors in mind when selecting a function.

> Plotting notes: In the plots we just made above, we use the `slot` function to grab the polygon IDs from each of the unioned `SpatialPolygons*`, and then use the `text` function, in this case a `generic` method from the `raster` package (there are other method for plotting text into other kinds of plots, e.g. xy scatter plots), to label the polygons in the map with the polygon labels. The visualization helps to understand some differences in the various unioning methods.   
...
```{r}
# Block 1
farmers_alb <- spTransform(farmers, proj4string(roads))  # 1
long_roads <- roads[which(road_lengths > 500000), ]  # 2

# Block 2
bwidths <- c(10000, 20000, 30000) # 1
set.seed(1) # 2
n <- sample(1:nrow(farmers_alb), size = 5, replace = TRUE)  # 3

# Block 3
buffs <- lapply(bwidths, function(x) {  # 1
  fbuf <- buffer(x = farmers_alb[n, ], width = x, dissolve = FALSE)  # 2
  rbuf <- buffer(x = long_roads, width = x, dissolve = FALSE)  # 3
  list("farm" = fbuf, "road" = rbuf)  # 4
})  # 5

# Block 4
par(mar = c(0, 0, 0, 0))  # 1
cols <- c("blue", "green", "yellow")  # 2
cols2 <- c("purple", "green4", "antiquewhite")  
plot(aggregate(districts_alb2, dissolve = TRUE), col = "grey")  # 3
for(i in length(buffs):1) {  # 4
  plot(buffs[[i]]$farm, add = TRUE, col = cols[i])  # 5
  plot(buffs[[i]]$road, add = TRUE, col = cols2[i])  # 6
}
```

There is a lot of code above, but I am not going to explain it this time. Rather, I am going to give you two exercises to do with it: 

<span style="color:blue">Exercise 14</span><a name="exercise14"></a>: ___Explain what each of the 4 code blocks is doing, line by line. The buffering operation is happening in one of them. We will go over this in class.___

<span style="color:blue">Exercise 15</span><a name="exercise15"></a>: ___Re-run the code above, except you should first replace the buffering function used above with the `gBuffer` function from rgeos. And then, once you have done that, run it again, but this time allow the buffering function to dissolve buffers where they intersect (either buffer approach will do that, so use the one you like best)___ [Exercise index](#unit3exercises)


# Unit Summary{#unitsummary}

This unit section has given a brief and by no means exhaustive introduction to do some common vector operations and ways in which vector data are manipulated. Along the way, it has also introduced some new plotting approaches and vocabulary, and has explored some of the issues regarding SpatialPolygons* structuring. 

There are a number of other types of operations that we could have looked at, some or many of which we will undoubtedly encounter as we plunge ahead.  
...
# Exercise 12
l <- list(spp_int_distrs2, spintd_agg4)
do.call(rbind, lapply(1:length(l), function(x) {  # x <- 1
  p <- slot(l[[x]], "polygons")  # get polygons slot from x
  ps <- t(sapply(1:length(p), function(y) {  # loop through p's poly groups
    a <- length(slot(p[[y]], "Polygons"))  # find out # polys in each poly group
    c("poly group" = y, "N polys in group" = a)
  }))
  cbind("object#" = x, ps)  
}))

# Exercise 13
sapply(slot(districts_int_spp, "polygons"), function(x) slot(x, "ID")) == 
  row.names(districts_int_spp)

# solution 1 - fix the polygon ids
districts_int_spp2 <- spChFIDs(districts_int_spp, 
                               x = as.character(row.names(districts_int_spp)))
erase(x = districts_int_spp2, y = spp_alb)

# solution 2 - fix the data.frame ids
districts_int_spp2 <- districts_int_spp
ids <- sapply(slot(districts_int_spp, "polygons"), function(x) slot(x, "ID"))
row.names(districts_int_spp2) <- ids
erase(x = districts_int_spp2, y = spp_alb)

# Exercise 15
# Block 1
farmers_alb <- spdat$farmers  # 1
long_roads <- roads[which(road_lengths > 500000), ]  # 2
...
# Block 3
buffs <- lapply(bwidths, function(x) {  # 1
  fbuf <- gBuffer(spgeom = farmers_alb[n, ], width = x)#, byid = TRUE)  # 2
  rbuf <- gBuffer(spgeom = long_roads, width = x)#, byid = TRUE)  # 3
  list("farm" = fbuf, "road" = rbuf)  # 4
})  # 5
