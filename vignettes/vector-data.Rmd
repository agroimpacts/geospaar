...
In the code above, you will see that we slightly revised the code, by putting the `gIntersects` inside the `which` function, and capturing the output in `spp_alb_int`, which we then used to subset `districts_alb2` into a new `SpatialPolygonsDataFrame`, `districts_int_spp`. 

In the plot commands function, you will see a new way of selecting colors, using the `rainbow` function. In this case, we tell `rainbow` to select "n" colors (here equal to the number of districts in `districts_int_spp`), so that we get a different color for each of the selected districts. We also overlay `spp_alb` on the districts it intersects, using angled cross-hatching in place of a fill color (set using the "density" and "angle" arguments). Cross-hatching is apparently considered a no-no, however, because of visual distortions, but now you know how to do it. 
...
Pretty clear what `gIntersection` does, and how it differs from `gIntersects`. However, note that you have to use the "byid = TRUE" option if you want to the detail of the intersected district boundaries, else you would get this.
...
Now, a much friendlier way of doing an intersection, which uses, somewhat counterintuitively, the `raster` package.  The developers of the `raster` package have created a more user friendly set of certain `rgeos` functions, so we are going to demonstrate those here. 
...
It turns out that `raster` has a function (`intersect`) that does the same job as `rgeos` (go ahead and plot `spp_int_distrs2`, and you will see), but goes further, in that it retains the relevant information from any `data.frame`(s) in the objects being intersected, whereas `gIntersects` only returns the intersected geometries, and you need to do a great deal more work to recover and then join the accompanying attributes to the geometries.  


> An aside: These last two example demonstrates both a strength and weakness of R, in my opinion--R's strength is that you can usually find multiple solutions to solve a particular problem; R's weakness is that choosing the best solution can be tricky, given the profusion of choice, and you might end up trying something that takes up a lot of your time. 

### Aggregation

```{r}
# plot(aggregate(x = spp_int_distrs2, by = "ID"))
# spp_int_distrs2$distName
# 
# gUnionCascaded()
# 
# aggregate(x = districts_alb2, y = spp_alb, 
#           by = list(i = "area"), FUN = sum)
# spp_int_distrs <- gIntersection(spgeom1 = spp_alb, spgeom2 = districts_alb2, 
#                                 byid = TRUE)
# rbind(districts_alb2, spp_alb)
# 
# 
# lapply(spp_int_distrs@polygons, function(x) {
#   x
#   slot(x, "ID")
# })
# 
# sapply(spp_int_distrs@polygons, function(x) slot(x, "ID"))
# 
# 
# SpatialPolygonsDataFrame(spp_int_distrs, 
#                          data.frame("distName" = districts_int_spp$distName))
