...
We have three variants on `sp::aggregate` shown above.  `spint_agg` resulted from "by = "ID"", which aggregates polygons according to the values the "ID" column. `spint_agg2` did not have that argument passed to it, `spint_agg3` had "dissolve = FALSE" as an argument, and `spint_agg4` is the same as the previous plus the "by = "ID"" argument. The plots shows that the first two look identical and that they were dissolved. The second two look like they weren't dissolved at all. So what are the differences? 
...
Plotting tip: note in the code above how we add titles to the plots, and also tweak the margins to add space to the top margin to help it display. 
