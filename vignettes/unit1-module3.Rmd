...
1. Create the following vectors: 
    ...
    - A date vector (named `v4`), using random sample without replacement between 2022/09/01 and 2022/12/15.
...
6. A further look into the `*apply`'s returns:
    - Make three data.frames for this experiment:
        - `df1` using `v1` and `v2` as columns
        - `df2` using `v2` and `v3` as columns
        - `df3` using `v3` and `v4` as columns
    - Combine `df1`, `df2`, and `df3` into a list `l`
    - Write an `lapply` that does nothing but return `df1`, `df2`, and `df3` each time
        - Hint: you will need to define an anonymous function that just returns its input
    - Do the same but with `sapply`
    - Compare the results from `sapply` and `lapply`
