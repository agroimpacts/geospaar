...
We'll round out this section with some additional raster-based analyses and a bit of modeling. Use the hidden code to create the objects you need from prior sections. 
...
Also, we will use a new package in this section, `dismo`, so install that and then load it up.
```{r}
library(dismo)
library(raster)
library(geospaar)
...
You can explore those further at your leisure. In our example, we choose the aggregated version of the SRTM DEM, and specified that we wanted it for Zambia. 

### Pixel resolution/area 

The DEM we downloaed is in decimal degrees with a resolution of `r raster::res(dem)`, which means the area of each pixel varies through Zambia. To illustrate that, we use a a nice function that `raster` provides that lets you calculate the area of each grid cell, even when it is in a geographic coordinate system:
...
The result is in km$^2$, and you can see that the area of each pixel falls as with increasing distance from the poles.  This is why we use equal area projections to calculate area values. 
...
The example above gives a hint at what can be done, and also shows how it can be somewhat complicated; it always takes us a ton of time and a number of internet searches to figure out how to do this. We won't explain this in detail here, but the code above provides a template that you can hack at if you want to get fancy with labelling your plots. Please ask in class if you want any further explanation.  
...
So we create (in Block 1) a 1 km projected version of the DEM, allowing the default bilinear interpolation to do its work during reprojection. In Block 2 we then calculate slope, aspect, flow direction, and the topographic ruggedness index. The commented out code illustrates the individual calculation of each variable, but we did it programmatically with an `lapply`. Note that in the `lapply` we pass the "unit" argument into the calculation of all four variables, even though it only is used for slope and aspect.  It has no affect on flow direction or TPI. 
...
Interpolating between point values is a fairly common GIS analysis. The `interpolate` function of raster allows you to do this using a number of different types of models, such as kriging and inverse distance weighting. We are going to look at examples of these two using functions drawn from the `gstat` package (please install it), and applied to the `raintot` data.
...
m
ordkrig <- gstat(id = "rain", formula = rain ~ 1, data = rainsamp, model = m) #5
ordkrigr <- interpolate(object = r, model = ordkrig)  #6
ordkrigrmsk <- mask(x = ordkrigr, mask = raintotalb)  #7
...
This is somewhat complex obviously, both conceptually and with code. I will refer you to other, non-R literature to explain [interpolation](http://gisgeography.com/inverse-distance-weighting-idw-interpolation/) and [kriging](http://gisgeography.com/kriging-interpolation-prediction/) if it is unfamiliar. 

The code above provides a demonstration of how you implement these two common spatial prediction models. 
...
Block 2 then draws a random sample from across the reprojected rainfall raster to represent point observations of rainfall.  

Block 3 runs the code for an inverse distance weighted (IDW) interpolation, i.e. each cell between points is given a new value that is the weighted mean the values from the nearest points, with the weight being determined by the distance of each point. The interpolation model is set up by the `gstat` function, using the coordinates x and y as the locations. `raster::interpolate` takes the model and applies it to `r`, making the new `invdistr` raster, which we then mask to the outline of Zambia (`invdistrmsk`). 

Block 4 implements ordinary kriging, which has additional steps. In #1 and #2, we have to first convert `rainsamp` from a `data.frame` into a `SpatialPointsDataFrame`. #3 calculates the variogram, which measures the spatial autocorrelation between rainfall values--this requires a spatial object, hence the need for lines #1 and #2. # 4 fits a spherical variogram model. #5-#7 then reprise the steps shown in Block 3. 

Block 5 is the now familiar stack and plot.  

<span style="color:blue">Exercise 14</span><a name="exercise15"></a>: ___Using just the code in blocks 1-3 as your template, create IDW interpolations using the original 1000 randomly sampled points, and then two new ones based on 1) 500 and 2) 250 randomly selected points. Create a 4 panel plot by a) stacking, in this order, `raintotalb`, 1000 point IDW, 500 point IDW, 250 IDW, b) naming the layers of the stack, c) using a single call to `plot_noaxes`. Use a "zlim = c(0, 150)" There are two ways of doing this: i) the hard way, by copying, pasting, and changing code blocks 2-3 for each version of the IDW; ii) the elegant way, by creating the three IDWs in an `lapply`. Note: if you choose the elegant way, to make the stack, you will first have to stack the list of three idws, and then stack that with `raintotalb`___ [Exercise index](#unit3exercises)
