...
Let's move on and look at some additional raster-based analyses. This section, as with the previous, will depend on objects created in earlier sections. The code to recreate those is here found hidden in the RMarkdown below.
...
```{r cutfig, echo=FALSE, eval=FALSE}
# Chunk 38
...
png(here::here("vignettes/fig/u2m2_categorize.png"), height = 400, 
    width = 600)
...
knitr::include_graphics("fig/u2m2_categorize.png")
```
...
usethis::use_data(raintot, overwrite = TRUE)

# save for later
raintotalb <- projectRaster(from = raintot, res = 5000, crs = crs(roads))
usethis::use_data(raintotalb, overwrite = TRUE)
```
...
```{r distancefig, echo=FALSE, eval=FALSE}
set.seed(1)
...
png("vignettes/fig/u2m2-distance.png", height = 400, width = 850)
...
```{r predictfig, eval=FALSE, echo=FALSE}
...
ggsave(p, filename = "vignettes/fig/u2m2_rfpredict.png", 
...
png("vignettes/fig/u2m2-rfpredicted_maps.png", height = 3, width = 7, 
    units = "in", res = 300)
