...
# Chunk 13
data("chirps")
# chirps <- brick(system.file("extdata/chirps.tif", package = "geospaar"))
```
...
```{r, echo=FALSE}
# hack workaround #1 because devtools and mask don't work at the moment
chirpsz <- brick(stack(sapply(1:nlayers(chirps), function(x) {
  mask(x = chirps[[x]], mask = districts)
})))
names(chirpsz) <- names(chirps)
```


```{r, fig.width=7, fig.height=2, eval=FALSE}
# Chunk 17
...
# hack workaround #2 because devtools and mask don't work at the moment
set.seed(1)
ind <- sample(1:nlayers(chirpsz), size = 3)
plot(chirpsz[[ind]], axes = FALSE, box = FALSE, nr = 1)
```
...
```{r, echo=FALSE}
# Another hack invisible chunk because devtools not allowing normal stacking
# behavior after R upgrade
rain_zmu <- calc(x = chirpsz, fun = mean)
rain_zsd <- calc(x = chirpsz, fun = sd)
rain_zrng <- calc(x = chirpsz, fun = range)
rain_zstack <- stack(rain_zmu, rain_zsd, rain_zrng[[1]], rain_zrng[[2]])
...
# Hack invisible chunk to allow plotting
plot_noaxes(rain_zstack)
```
...
farmers_env <- farmers %>% distinct(uuid, .keep_all = TRUE) %>% 
  select(uuid, x, y) %>% st_as_sf(coords = c("x", "y")) %>% 
...
rain_stats <- bind_rows(
  farmers_env %>% drop_na %>% select(rain) %>% mutate(dat = "Farmers"), 
