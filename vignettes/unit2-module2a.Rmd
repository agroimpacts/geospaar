...
```{r datasets, message=FALSE, warning=FALSE, eval=FALSE}
library(geospaar)
...
knitr::include_graphics("fig/u2m2_rasterlayer.png")
```
...
#> [49]  32  14   2  45  18  22  78  65  70  87  70  75  81 100  13  40
...
```{r dummybrick, fig.width=7, fig.height=2.5, eval=FALSE}
# Chunk 4
s <- stack(l)
...
s
#> class      : RasterStack 
#> dimensions : 8, 8, 64, 3  (nrow, ncol, ncell, nlayers)
#> resolution : 0.25, 0.25  (x, y)
#> extent     : 27, 29, -16, -14  (xmin, xmax, ymin, ymax)
#> crs        : +proj=longlat +datum=WGS84 +no_defs 
#> names      :         r,        r2,        r3 
#> min values :    1.0000,    0.0000,    5.5706 
#> max values : 100.00000,   1.00000,  14.80324
...
knitr::include_graphics("fig/u2m2_dummybrick.png")
```
...
```
```{r, echo = FALSE, out.width="100%"}
knitr::include_graphics("fig/u2m2_distplots.png")
```
...
#> class      : RasterLayer
#> dimensions : 98, 117, 11466  (nrow, ncol, ncell)
#> resolution : 0.1, 0.1  (x, y)
#> extent     : 21.99978, 33.69978, -18.02621, -8.226213  (xmin, xmax, ymin, ymax)
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_rasterize.png")
```
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_rasterize2.png")
```
...
knitr::include_graphics("fig/u2m2_projections1.png")
```
...
```
```{r, echo = FALSE, out.width="80%"}
knitr::include_graphics("fig/u2m2_bilinear_vs_nn.png")
```
...
```
```{r, echo = FALSE, out.width="100%"}
knitr::include_graphics("fig/u2m2_chirpsplot.png")
```
...
```
```{r, echo = FALSE, out.width="100%"}
knitr::include_graphics("fig/u2m2_mask3.png")
```
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_crop.png")
```
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_zoom.png")
```
...
```{r plot_noaxes_plot, eval=FALSE}
# Chunk 21
plot_noaxes(chirpsz[[1]])
# chirpsz[[1]] %>% plot_noaxes  # also works in a pipeline
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_plot_noaxes_plot.png")
```
...
chirpsz1agg
#> class      : RasterLayer 
#> dimensions : 99, 117, 11583  (nrow, ncol, ncell)
#> resolution : 0.1, 0.1  (x, y)
#> extent     : 21.95, 33.65, -18.1, -8.200001  (xmin, xmax, ymin, ymax)
#> crs        : +proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0 
#> source     : memory
#> names      : Y16299 
#> values     : 0, 12.18843  (min, max)
```
...
chirpsz1km
#> class      : RasterLayer 
#> dimensions : 985, 1170, 1152450  (nrow, ncol, ncell)
#> resolution : 0.01, 0.01  (x, y)
#> extent     : 21.95, 33.65, -18.05, -8.200001  (xmin, xmax, ymin, ymax)
#> crs        : +proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0 
#> source     : memory
#> names      : Y16299 
#> values     : 0, 18.00534  (min, max)
```
...
```{r agg_extenterror, error=TRUE, eval=FALSE}
# Chunk 23
...
knitr::include_graphics("fig/u2m2_agg_extenterror.png")
```
...
```
```{r, echo = FALSE, out.width="80%"}
knitr::include_graphics("fig/u2m2_resample.png")
```
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_fixnas.png")
```
...
#> 1st Qu.     0.00000     0.00000     0.000000
#> Median      0.00000     0.00000     0.000000
#> 3rd Qu.     0.00000     0.00000     0.000000
#> Max.       18.00534    17.76521     8.853778
#> NA's    20995.00000 20995.00000 20995.000000
```
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_cellstats2.png")
```
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_cvplot.png")
```
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_histogram.png")
```
...
head(zonemu)[, 1:5]
#>      zone     Y16299    Y16300    Y16301    Y16302
#> [1,]    1 0.00000000 0.0000000 0.0000000 1.8976312
#> [2,]    2 0.01322501 0.0000000 0.0000000 0.0000000
#> [3,]    3 6.04584049 1.8021238 0.0000000 4.4494707
#> [4,]    4 0.00000000 0.3011147 0.0201552 0.0000000
#> [5,]    5 6.25742351 3.6532615 1.0868976 0.4739815
#> [6,]    6 0.00000000 0.0000000 0.0000000 0.0000000
```

That creates a matrix (truncated here to show the first 6 rows and 5 columns) of mean rainfall within each zone ( district), by date. The first attempt to run `zonal` (#1) would have failed because of mismatched extents (and therefore was commented out so it wouldn't run), so we used `resample` in #2 to align extents before re-running `zonal`. 
...
```
```{r, echo = FALSE, out.width="60%"}
knitr::include_graphics("fig/u2m2_subs.png")
```
...
```
```{r, echo = FALSE, out.width="80%"}
knitr::include_graphics("fig/u2m2_focal.png")
```
...
```{r, echo=FALSE, out.width="80%"}
knitr::include_graphics("fig/u2m2_calc.png")
```
...
***
<center>
[To Unit2 Module 2b](unit2-module2b.html) || [Back to top](#working-with-raster-data) || [Back to **vignette index**](toc.html) 
</center>
***
