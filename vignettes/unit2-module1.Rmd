...
`geospaar` imports `sf`. `sf` depends on the following external libraries (from [here](https://cran.r-project.org/web/packages/sf/index.html)): 
...
We used `file.remove` to get rid of the .sqlite, and then used `rm` to get rid of the existing `farmers` `r class(farmers)[1]` object, and then read back in the `farmers` we had written to the .shp. So we have `farmers` back again.   
