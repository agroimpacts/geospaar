...
- First, it is useful to appreciate the connection between generic functions, which work with both S3 and S4 classes. If you have ever worked with python, you end up doing something like this:
  ...
  Where the method (the function `getcwd`) appears after the object (`os`), because it belongs to the class. The generic functions in R are defined for the class, and when you execute it behaves in a manner specific to the class--assuming a generic has been defined explicitly for the class of object to which you are trying to apply it. If a class generic method doesn't exist, R will apply the default version of the function (e.g. `print.default()`).  
