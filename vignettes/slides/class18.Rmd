...
```{r, eval = FALSE}
rainsd <- calc(chirpsz, fun = sd)
r1 <- rainsd < cellStats(rainsd, mean)
rainsd[r1 == 0] <- NA
plot_noaxes(rainsd, main = paste("Avg. SD=", round(cellStats(rainsd, mean), 2)))
```
...
```{r, eval = FALSE}
qtiles <- quantile(raintot, probs = seq(0, 1, 1 / 5))
raintotcut <- cut(x = raintot, breaks = qtiles, include.lowest = TRUE)

rclmat <- cbind(qtiles[1:5], qtiles[2:6], 1:5)
rclmat
raintotrcl <- reclassify(x = raintot, rcl = rclmat, include.lowest = TRUE)

cols <- c("tan", "yellow2", "yellowgreen", "green3", "green4")
s <- stack(raintotcut, raintotrcl)
names(s) <- c("Cut", "Reclassify")
plot_noaxes(s, legend = FALSE, col = cols, mar = c(0, 0, 1, 0))
legend(x = "bottomright", legend = c("Q5", "Q4", "Q3", "Q2", "Q1"),
...
```{r, eval = FALSE}
# select  district ids at random
seed <- 11
ssize <- 300
set.seed(seed)
distids <- sample(districts$ID, size = 15, replace = FALSE)

# mask out those districts
randdistsr <- distsr_rs %in% distids
randdistsr <- mask(x = distsr_rs, mask = randdistsr, maskvalue = 0)
# plot(randdistsr)

# random sample from districts
set.seed(seed)
distsamp <- sampleRandom(x = randdistsr, size = ssize, cells = TRUE)
# head(distsamp)
newrandrain <- raintot[distsamp[, 1]]

# stratified
set.seed(seed)
distsamp_str <- sampleStratified(x = randdistsr, size = ssize / length(distids),
                                 cells = TRUE)
stratrandrain <- raintot[distsamp_str[, 1]]

# district mean rain
randdists <- districts[distids, ]  # 4
randdistrain <- extract(x = raintot, y = randdists, fun = mean)

# plot
randmu <- mean(randrain, na.rm = TRUE)
strrandmu <- mean(stratrandrain, na.rm = TRUE)
randdistrainmu <- mean(randdistrain, na.rm = TRUE)
boxplot(x = list(randdistrain, randrain, stratrandrain), ylab = "mm",
        col = c("blue", "red", "purple"), xaxt = "n", main = "Rainfall")
points(x = 1:3, y = c(randdistrainmu, randmu, strrandmu), pch = 16, cex = 2)
