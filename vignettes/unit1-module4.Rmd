...
We use `tidyr::pivot_wider` to do that, specifying the *Element* column as the one that contains the names of the new columns (`names_from`), and *Value* as the column from which we take the values to fill under each column (`values_from`). Note that this function replaces the commented-out one below it, which is the original `tidyr::spread` function. `pivot_wider` is considered more intuitive. The inverse procedure is `tidyr::pivot_longer`, which is also known as **gathering** (and the older function is `tidyr::gather`), where columns are re-organized into key-value pairs: 
```{r}
# Chunk 7
maize %>% gather(key = Element, value = Value, `Area harvested`, Production)
maize %>% pivot_longer(cols = c("Area harvested", "Production"), 
                       names_to = c("Element", "Value"), names_sep = "")
```
