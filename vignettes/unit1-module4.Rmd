...
## Vignette tasks
1. Create three `tibble`s, `t1`, `t2`, `t3`. `t1` has 10 rows, with column `V1` containing values G1 through G10, and `V2` containing `runif` between 75 and 125.  `t2` has 15 rows with `v1` (G1 - G15).  `v3` containing a weighted random selection of `LETTERS` A-F, which A-C have about 80% chanced to be selected whereas D-F have about 20% chance to be selected. `t3` has 20 rows (`v1` with G1-G20), and `v4` with numbers from the random normal distribution (`mean` = 100, `sd` = 20). Use a seed of 1 for random numbers. Join `t1`, `t2`, and `t3` within a single pipeline, using:
  - `left_join`
  - `right_join`
  - `full_join`
  - `inner_join`
