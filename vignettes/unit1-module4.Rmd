...
## Analyses
Now that we have a sense of how to do split-apply-combine, let's dig a bit more into the analyses that we can do as part of that process. Since `R` was originally designed for statistics, there are many possible analyses we can do with `R`. We'll look at just a few common ones.

### Summaries
We'll start with some summary statistics outside of SAC. The easiest way to do that in `R` is to use the base `summary` function. 
...
## Multi-panel plots
There is more to this story with the divergent yield trends, and it requires multiple plots to really tell it. Let's have a look at yield, production, and harvested area trends over time to see what's going on:
...
## Practice
### Questions
1. How do the axis labels in the two examples given in Chunk 41 differ from the plot in Chunk 40 (run the code to check)? 

2. What happens when you run `ggplot` without a `geom_*` argument? 
