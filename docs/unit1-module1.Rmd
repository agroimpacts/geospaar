...
As the first module, we will first take a brief overview of R language and begin to learn about the fundamentals of `R` language, starting with the different types of `R` objects, and how and where (in environments) they interact with one another.
...
Another awesome overview of R development is this "[20 Years of R](https://www.youtube.com/watch?v=iq_biXEIx-U&feature=emb_logo)" introduced by David Smith. 
...
The super assign symbol `<<-` can be used within an execution environment to modify a global object. You have to use it with caution because it is very easy to lose control. 
...
Note that the value of `x` within global environment doesn't change after running function `f1`, but got updated after running function `f2`.
