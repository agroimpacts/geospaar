...
```{r, eval=FALSE}
library(parallel)
system.time(b3 <- lapply(1:500, function(x) {
...
system.time(b4 <- lapply(1:500, function(x) {
  r <- raster(extent(30, 31, -14, -13), res = 0.001, 
              crs = "+proj=longlat +datum=WGS84")
  set.seed(x)
  values(r) <- sample(1:10, size = ncell(r), replace = TRUE)
...
system.time(l1 <- lapply(b3[1:10], function(x) calc(x, mean)))
system.time(l2 <- mclapply(b3, function(x) calc(x, mean), mc.cores = 4))
