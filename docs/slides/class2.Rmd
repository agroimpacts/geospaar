...
- `length`, and more specifically `dim` for matrix and array. Resetting `dim` attribute can convert a vector to matrix/array or reconstruct a matrix/array. 

  * When we use `length` to a matrix or array, it will give you the whole number of items.
  
- `class`: it is a bit different from function `class`.
- function `attr(object, attr_name)` can be used to query or set an attribute of an object. This is super useful when we want to add some meta data for an object.
