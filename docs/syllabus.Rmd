...
## Meeting Time/Place
- GRACE, MW 12:00PM - 1:15PM
- JC103 (Lab), F 01:25PM - 02:40PM or 02:50PM - 04:05PM
...
# a / sum(a)
instructor <- 'Lei Song'
hours_instructor <- 'Tuesday 02:00PM - 04:00PM (Virtual appointments only)'
ta <- 'Pilar Delpino Marimón'
hours_ta <- 'Wednesday 04:00PM - 06:00PM (Virtual appointments only)'
hours <- rbind(c(instructor, hours_instructor), 
            c(ta, hours_ta))
colnames(hours) <- c("Instructor/TA", "Office hours")
kbl(hours, escape = F, align = "l") %>%
...
## Class materials
- [Class materials web page](https://agroimpacts.github.io/geospaar/)
- [Class materials R package](https://github.com/agroimpacts/geospaar)
- [Class GitHub group](https://github.com/agroimpacts)
...
__Projects__ Each student will be required to undertake a final project. Please see the [projects](https://agroimpacts.github.io/geospaar/projects.html) page for more detail.
...
# kable(mat)
kbl(mat, escape = F, align = "l") %>%
  kable_paper("striped", full_width = F,
              html_font = "Helvetica Neue") %>%
  kable_styling(font_size = 16) 
```

Grading will be based on the rubrics found under the [Assessment](https://agroimpacts.github.io/geospaar/assessment.html) vignette.
...
### Communications
We will conduct class communications via a Slack channel that you should already be invited to. ***Please don't send emails as they will go unanswered***. Class-wide discussions will be conducted in the #sprint2021 channel. Individual and restricted group messaging will be conducted via Slack direct messaging, e.g. grade reporting, confidential questions.  

# Course Structure{#structure}
The following is an overview of the course structure. Note that this structure will evolve during the semester, and we will adjust material to match the pace of learning. 

```{r timetable, echo = FALSE, warning=F, message=F}
library(kableExtra)
library(dplyr)
start_date <- as.Date('2021-02-22')
timetable <- data.frame(Week = 1:14) %>% 
  mutate(day1 = format(start_date + (Week - 1) * 7, '%b-%d'),
         day2 = format(start_date + Week * 7 - 1, '%b-%d')) %>% 
  mutate(Date = paste(day1, day2, sep = ' - ')) %>% 
  dplyr::select(-c(day1, day2)) %>% 
  mutate(Objective = c('R and R fundamentals',
                       'Reproducibility and related R skills',
                       ...
                       'Project work continued',
                       'Course completion week'))
due_date <- timetable$Date[c(2, 4, 5, 7, 9, 10)]
timetable$Date <- cell_spec(timetable$Date, 
                            background = ifelse(timetable$Date %in% due_date, 
                                           "red", ''))
kbl(timetable, escape = F, caption = "Timeline") %>%
  kable_paper("striped", html_font = "Helvetica Neue") %>%
  kable_styling(font_size = 16)  %>% 
  pack_rows("Unit 1 An introduction to R and related reproducibility skills", 
            1, 4, label_row_css = "background-color: #666; color: #fff;") %>%
  pack_rows("Unit 2 Handling and analyzing spatial data with R", 
            5, 9, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Unit 3 Projects", 
            10, 14, label_row_css = "background-color: #666; color: #fff;") %>% 
  footnote(general = "The red cell indicates assignment due week (Friday midnight).")
```
...
This syllabus provides the general subject for each week and class. The detailed readings and assignments for each week and day can be found in the accompanying [Unit 1 vignette](https://agroimpacts.github.io/geospaar/unit1.html), as well as the overall learning goals for the unit.  

**Week 1. R and R fundamentals**

**Week 2. Reproducibility and related R skills**

- Unit 1 Module 1 assignment (#1) due

**Week 3. Data preparation and operation**
...
In this part of the course we will start to learn to use R as a GIS.  The detailed syllabus can be found in the [Unit 2 vignette](https://agroimpacts.github.io/geospaar/unit2.html).
