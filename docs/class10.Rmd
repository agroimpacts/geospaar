...
```{r, eval=FALSE}
output_folder_path <- "C:/Users/micha/Documents/geospaar/external/"
...
for(current_site in sites){
  ...
  write.csv(site_obs, file = paste0(output_folder_path, csv_name))
}
...
```{r, eval=FALSE}
l <- lapply(iterator, function(item){
  ## do stuff
  return(something)
})
```
