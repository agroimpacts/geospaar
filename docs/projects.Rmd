...
# Potential project list (new ones)

## Urbanization and gentrification
**Provided by Pilar Delpino Marimón**

What is the next urban frontier? Gentrification is a process
of socio-economic change at the neighborhood level. Through historical analysis
of real estate investment and income of residents, one could map how the economic
composition of a city changes. This often leads to the displacement of low-income
classes. Some argue gentrification produces spatial inequalities.
If you are interested in gentrification and urbanization,
your team can approach this question through a spatial justice lens. Where are resources
distributed? Which neighborhoods are favored over others in terms of
investment? Example of playground distribution and child population: http://spatialjustice.org/toolbox/analyzing-access-to-resources

A different way to engage with gentrification can be by
doing a historical spatial analysis of economic changes in the population of
the city and identifying hot spots of change. The analysis could be done at a
census tract and do a spatial comparison based on income, poverty, and property
changes throughout time. Can you map areas in Worcester that have been
gentrified in the last 30 years? (UK
example: https://carto.com/blog/spatial-analysis-gentrification/).

Consider exploring this page: https://www.brown.edu/academics/spatial-structures-in-social-sciences/ltdb-following-neighborhoods-over-time

## Electric vehicles and social justice

A group of students did this work before. Here is their repo: https://github.com/agroimpacts/electric.

The background information took from their repo:

This project examines the accessibility of electric vehicle (EV) usage in the United States through the lens of justice and equity. Accessibility is measured through monetary value, distance, and access to charging stations. Social justice and equity is quantified using:

(1) Socioeconomic data and its correspondence to the spatial distribution of EV ownership through car registrations;

(2) And identifying gaps in service provision. 

## Public health inequalities across counties
**Provided by Pilar Delpino Marimón**

You could map service areas in relation to population
density. Or add more complexity by identifying a particular disease and calculating
clusters. You could do an air quality analysis and compare that existing
conditions, policies for clean air, etc. 

This paper is a great example, you could
use it as inspiration: https://ij-healthgeographics.biomedcentral.com/articles/10.1186/s12942-020-00242-0

Information on GIS and public health: https://www.cdc.gov/gis/index.htm

## Network analysis or a transportation related analysis
**Provided by Pilar Delpino Marimón**

The Bureau of Transportation Statistics has a comprehensive
database that could be used to do network analysis and other transportation
related spatial analysis: https://www.bts.gov/maps.

You could do a project on spatial inequality based on access to public transportation,
areas that are underserved, etc. 

For more ideas visit: http://spatialjustice.org/issues/transportation-issues/

## Auto-segment agricultural field

This is a expand of my current work. It might be a bit experimental and challenging. The general idea is:

1. To under-segment the satellite image to get the edges;

2. Overlay the existing LC/cropland map to select out the edges of farms;

3. Run an abnormal detection method to calculate the probability of being edges;

4. Use this probability map back to the satellite image to run segmentation again.

## Temporal profile of crop types

With the availability of time-dense Earth observation, we could monitor and analyze more detailed landscape characteristics. For instance, the time-series of cloud-resistant Sentinel-1 images can help us to distinguish different crop types. You could do a project to:

1. Extract temporal profile of each crop types.

2. Use profile matching method to predict crop types over an area.

Here are public crop type datasets that you can use: https://www.mlhub.earth.

## Species distribution modeling (SDM) and/or habitat connectivity

If you are interested in SDM, we could design a workflow based on your interests. Because we could get public dataset for many species over the world.

If this is a group project, we could also apply method like circuit theory to generate habitat connectivity based on environmental suitability.

# Potential project list (old ones)
## Add on to our COVID-19 tracker
Have a look at our existing [repo/package](https://github.com/agroimpacts/covid19clark), and some of the initial dashboards our previous class made. Potential projects include converting the current code to an operational shiny dashboard (as opposed to a local one) and integrating more measures. For example, write a scraper to ingest the [Mass DPH's weekly town-level](https://www.mass.gov/info-details/covid-19-response-reporting#covid-19-daily-dashboard-) case rates and map these. 
