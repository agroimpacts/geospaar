...
# package (external)/namespace (internal)
environment(lm)
environment(ls)
...
`library()` attaches packages to the `search()` path
```{r}
#| echo: true
#| 
search()
...
`search()` order is important for understanding which exported functions are found first, and which are masked.
...
set.seed(1)
s2 <- sample(letters[1:7], size = 20, replace = TRUE)
s2_unique <- unique(s2)
s2
print(length(s2))
