...
```
---
## [Let's - a -go](https://www.youtube.com/watch?v=UiIRlg4Xr5w)
...
```{r, eval = FALSE}
for(k in 1:10){
  DF[k, ] <-         ## fill in code here for the kth row
...
```{r, eval = FALSE}
average_finish <- apply(...) ## fill in code for apply
```
...
```{r, eval = FALSE}
DF$first_place <- apply() ## fill in code
```
