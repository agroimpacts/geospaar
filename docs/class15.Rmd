...
(2) Create a buffer of width 0.5 (50 km) around that
```{r, eval = FALSE}
sf_use_s2(TRUE)
smallest_buffer <- smallest_centroid %>% st_buffer(dist = 55000)
ggplot() + geom_sf(data = smallest_buffer)

sf_use_s2(FALSE)
smallest_buffer2 <- smallest_centroid %>% st_buffer(dist = 0.5)
ggplot() + geom_sf(data = smallest_buffer) + 
  geom_sf(data = smallest_buffer2, fill = "transparent", col = "red")
```
...
p + geom_sf(data = dists_in_buff, fill = "grey30") + 
  geom_sf(data = smallest_buffer, col = "blue", fill = "transparent")

dists_in_buff2 <- districts3 %>% 
  filter(st_intersects(., smallest_buffer2, sparse = FALSE))
p + geom_sf(data = dists_in_buff2, fill = "grey30") + 
  geom_sf(data = smallest_buffer, col = "blue", fill = "transparent")
```
...
```{r, eval=FALSE}
area_buff <- buff_int_dists %>% 
  mutate(int_area = as.numeric(units::set_units(st_area(.), "ha"))) %>%
  select(distName,int_area)
ggplot() + geom_sf(data = area_buff, aes(fill = log10(int_area))) 
```
...
for(i in 1:4) {
  crop_ylds %>% filter(country == "ZAF" & crop == "Maize") %>% 
    ...
    plot(yields ~ year, data = ., cex = i, col = cols[i], axes = FALSE)
}
# multiple lines
par(mar = rep(3, 4))
...
axis(side = 1, las = 2, mgp = c(0.1, 0.5, 0), cex.axis = 0.8)
mtext("year", side = 1, line = 2)
axis(side = 2, las = 2)
mtext("yield", side = 2, line = 2)
