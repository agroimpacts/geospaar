...
There is a pure tidyverse way of doing this, which is the last thing to look at here.  It is more involved than previous examples, because the non-tabular (list) output from `lm` prevents use of `lm` within `dplyr::summarise` directly:
```{r, eval = FALSE}
# Chunk 39
crop_ylds %>% nest_by(crop, country) %>% 
  # Must set data = data here
  mutate(prod_ha_lm = list(lm(prod ~ harv_area, data = data))) %>% 
  summarise(tidy(prod_ha_lm))
#> # A tibble: 12 x 7
...
This produces a nice output table that lists the coefficients (estimate), standard errors, t-statistic, and p-value for each crop-country model. Each model occupies two lines: one for the model intercept results, one for independent variable results. There are a couple of extra functions, including `nest_by`, which is described by `?nest_by` as:

> ...is closely related to group_by(). However, instead of storing the group structure in the metadata, it is made explicit in the data, giving each group key a single row along with a list-column of data frames that contain all the other data.
...
1. Create three `tibble`s, `t1`, `t2`, `t3`. `t1` has 10 rows, with column `V1` (*uppercase*) containing values G1 through G10, and `V2` (*uppercase*) containing `runif` between 75 and 125.  `t2` has 15 rows with `v1` (*lowercase*) (G1 - G15) and `v3` (*lowercase*) containing a random selection of `LETTERS` A-F. `t3` has 20 rows (`v1` (*lowercase*) with G1-G20), and `v4` (*lowercase*) with numbers from the random normal distribution (`mean` = 100, `sd` = 20). Use a seed of 1 for random numbers. Join `t1`, `t2`, and `t3` within a single pipeline, using the following methods separately:
