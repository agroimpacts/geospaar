...
```{r, eval = F}
ggplot(data = up_data) +
  geom_point(aes(x = date, y = ndvi_hourly, size = ndvi), col = 'red') + 
```
...
```{r, eval = F}
ggplot(data = up_data) +
  geom_point(aes(x = date, y = ndvi_hourly, size = ), col = 'red') + 
```
...
all_sites_data <- rbind(up_data, mid_data, down_data)
...
ggplot(all_sites_data) + 
  geom_point(aes(x = date, y = ndvi_hourly)) 
```
...
```{r}
ggplot(data = up_data %>% filter(date <= as_date("2018-12-31"))) +
  geom_histogram(aes(x = ndvi_hourly)) 
```
...
points(mid_data$date, mid_data$ndvi_hourly, col = 'blue')
```
...
```{r}
library(cowplot)
plot_2020 <- (all_sites_data %>% filter(year == 2020)) %>% 
  ggplot(.) + geom_point(aes(x = date, y = ndvi_hourly, color = site_name)) +
  scale_color_manual(values = c("red", "blue", "green")) + 
  ylab("ndvi") + xlab("date") + 
  ggtitle("2020")

plot_2019 <- (all_sites_data %>% filter(year == 2019)) %>% 
  ggplot(.) + geom_point(aes(x = date, y = ndvi_hourly, color = site_name)) +
  scale_color_manual(values = c("red", "blue", "green")) + 
  ylab("ndvi") + xlab("date") +
  ggtitle("2019")
...
---
## Practice
- [ggplot color manual](http://sape.inf.usi.ch/quick-reference/ggplot2/colour )
- Create a GCVI plot for midslope pod, 2019. Pick a color from above.

- Use three `geom_point` commands to plot GCVI for each site in 2019. (mid, up, down etc. ). Pick three colors.

- Use `facet_grid` to create separate plots for each year. Each plot should contain all three sites.
