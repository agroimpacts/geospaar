...
- `arrange` function in `dplyr` to order the table based on column(s). `desc` function can be used to arrange decreasingly.
...
Read Unit1-Module3 and the help documentation of each function to learn how to use them.
...
Loop over crop types to do calculate max of each type of observations. You are expected to get a matrix back and assign to object `crop_smr`. Hint: You might need function `unique`, and write anonymous function and use `dplyr` within this anonymous function.
