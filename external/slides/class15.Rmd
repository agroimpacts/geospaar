...
crop_ylds %>% group_by(crop, country) %>% 
  ...
  select(crop, country, yields) %>% summarise_all(list(mu = mean, sd = sd))
...
crop_ylds %>% filter(crop == "Maize") %>% ggplot() + 
  geom_histogram(aes(x = yields, fill = country), bins = 15, col = "grey30",
                 position = "dodge") +
