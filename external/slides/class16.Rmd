...
p <- ggplot(districts3) + geom_sf(aes(fill = log10(n)))
```
...
districts3 %>% 
  ...
  districts3 %>% filter(n > 15) %>% 
  geom_sf(data = ., fill = "red")  + 
  districts3 %>% filter(between(n, 1, 15)) %>% 
  geom_sf(data = ., fill = "blue")
```
...
p + geom_sf(data = pols, aes(fill = as.factor(ID))) + 
  geom_sf(data = st_union(pols), fill = "transparent", col = "purple")
```
