...
---
## 1-d/2-d Indexing/subsetting/replacing
- Select from `m1$V2` all values > 100, and programmatically count how many values were selected
- Select the first 10 values from `m1`, using a single vector and no row or column information
- Use a single vector to select the last row, column value from `m1`
- Replace the value selected above with -99
- Now select row 3, columns 1:2 from `m1`, and replace them with their values multiplied by 10 
- Do the same, but select the columns by their name, and reset the new values by dividing by 10
- Select from `dat1` the values of V3, and square them. Do it using 1) index notation and column name using both 2) `[]`, and 3) `$`
- Subset the first two rows and columns of `dat1` into a new data.frame `datss`. 
- Replace `dat1` rows 1:2, column 1:2 with the values -1:-4
- Reset the part of `dat1` you just changed with the values in `datss`
