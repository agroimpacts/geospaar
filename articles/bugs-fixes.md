...
When installing `geospaar`, older versions of RStudio may cause a pandoc
failure when building vignettes. Here is a fix (courtesy of Lei Song).
...
Then restart Rstudio and proceed again with `devtools::install*` steps.
This step can be avoided if you update to the latest Rstudio, however.
